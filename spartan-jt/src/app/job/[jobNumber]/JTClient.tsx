...
function FinancialsTab({ job, data, amt, invTotal, paidTotal }: any) {
  ...
  return <>
    <div className="tab-hdr">
      ...
      <div className="tab-info"><div className="tab-title">Financials</div><div className="tab-desc">Deposits &middot; Financing &middot; Material costs &middot; Profitability</div></div>
    </div>
    <div className="hero">
    ...
    <div className="c full"><div className="ch"><h3>Invoices</h3><div className="tg" style={{ background: 'var(--firebg)', border: '1px solid var(--firebd)', color: 'var(--fire)' }}>{invoices.length}</div></div>
      {invoices.length > 0 ? <div className="cb" style={{ padding: 0 }}><table className="mt"><thead><tr><th>Invoice</th><th>Date</th><th>Subtotal</th><th>Tax</th><th>Total</th><th>Balance</th></tr></thead><tbody>
        {invoices.map((inv: any, i: number) => <tr key={i}><td style={{ fontFamily: 'var(--mono)', color: 'var(--ice)' }}>{inv.reference_number || inv.st_invoice_id}</td><td>{fmt(inv.invoice_date)}</td><td>${(parseFloat(inv.sub_total) || 0).toFixed(2)}</td><td>${(parseFloat(inv.sales_tax) || 0).toFixed(2)}</td><td style={{ color: 'var(--t1)' }}>${(parseFloat(inv.total) || 0).toFixed(2)}</td><td style={{ color: parseFloat(inv.balance) > 0 ? 'var(--fire)' : 'var(--mint)' }}>${(parseFloat(inv.balance) || 0).toFixed(2)}</td></tr>)}
    ...
    <div className="c full"><div className="ch"><h3>Payments</h3><div className="tg" style={{ background: 'var(--mintbg)', border: '1px solid var(--mintbd)', color: 'var(--mint)' }}>{payments.length}</div></div>
      {payments.length > 0 ? <div className="cb" style={{ padding: 0 }}><table className="mt"><thead><tr><th>Payment</th><th>Type</th><th>Amount</th><th>Date</th><th>Memo</th></tr></thead><tbody>
        {payments.map((p: any, i: number) => <tr key={i}><td style={{ fontFamily: 'var(--mono)', color: 'var(--ice)' }}>{p.st_payment_id}</td><td>{p.payment_type || '\u2014'}</td><td style={{ color: 'var(--mint)' }}>${(parseFloat(p.total) || 0).toFixed(2)}</td><td>{fmt(p.payment_date)}</td><td>{p.memo || '\u2014'}</td></tr>)}
    ...
    <div className="g2">
      <div className="c"><div className="ch"><h3>Deposit Tracking</h3></div><div className="cb">
        <VR dot={paidTotal >= deposit40 ? 'ai-ok' : 'ai-fail'} k="40% Required" v={money(deposit40)} />
        <VR dot={paidTotal >= deposit40 ? 'ai-ok' : 'ai-fail'} k="Collected" v={money(paidTotal)} style={{ color: paidTotal >= deposit40 ? 'var(--mint)' : 'var(--fire)' }} />
      </div></div>
      <div className="c"><div className="ch"><h3>18% Material Budget</h3></div><div className="cb">
        <VR dot="ai-ok" k="Budget Cap" v={money(amt * 0.18)} />
        <VR dot="ai-wait" k="Material Spend" v="Tracking..." style={{ color: 'var(--t3)' }} />
      </div></div>
    </div>
  </>;
}
...
function VerifyTab({ data, score, passed, failed, total }: any) {
  ...
  verifs.forEach((v: any) => {
    ...
    grouped[phase].push(v);
  });
  return <>
    <div className="tab-hdr">
      <div className="tab-icon" style={{ background: 'var(--hotbg)', border: '1px solid var(--hotbd)', color: 'var(--hot)' }}><Icon name="shield" /></div>
      <div className="tab-info"><div className="tab-title">Verification Dashboard</div><div className="tab-desc">Bird&apos;s-eye scorecard of ALL checks</div></div>
    ...
    {Object.entries(grouped).map(([phase, checks]) => {
      ...
      return <div className="sec" key={phase}>
        ...
        <div className="vg">{checks.map((c: any, i: number) => {
          ...
          return <div className="vg-item" key={i}><div className={`vg-dot ${dot}`} /><div className="vg-label">{c.verification_name}</div></div>;
        })}</div>
      </div>;
    })}
