...
// Strip catalog codes like "PRO45B - " or "34-600XL - " from item names
function cleanName(name: string): string {
  if (!name) return '';
  // Pattern: code prefix followed by " - " then the real description
...
export default function MaterialsTab({ job, data, amt }: { job: any; data: any; amt: number }) {
  ...
  return <>
    <div className="tab-hdr">
      ...
      <div className="tab-info">
        <div className="tab-title">Materials</div>
        <div className="tab-desc">{totalItems} items &middot; Generated {fmt(ml.generated_at)}</div>
      </div>
    </div>
    ...
    <div style={{ fontSize: 11, color: 'var(--t3)', textAlign: 'center', padding: '12px 0' }}>
      Generated by AI ({ml.ai_model || 'Sonnet 4.6'}) &middot; {fmt(ml.generated_at)}
