...
const LEVELS = [
  { min: 0, name: "Recruit", rank: "I", color: "#64748b" },
  { min: 100, name: "Trainee", rank: "II", color: "#3b82f6" },
  { min: 250, name: "Apprentice", rank: "III", color: "#8b5cf6" },
  { min: 450, name: "Journeyman", rank: "IV", color: "#f59e0b" },
  { min: 700, name: "Spartan", rank: "V", color: "#c8a84e" },
];
const CAT_META: Record<string, { label: string; color: string; bg: string }> = {
  "Pre-employment": { label: "PE", color: "#60a5fa", bg: "rgba(96,165,250,0.08)" },
  Documents: { label: "DC", color: "#a78bfa", bg: "rgba(167,139,250,0.08)" },
  Accounts: { label: "AC", color: "#22d3ee", bg: "rgba(34,211,238,0.08)" },
  Profile: { label: "PR", color: "#fbbf24", bg: "rgba(251,191,36,0.08)" },
  Equipment: { label: "EQ", color: "#818cf8", bg: "rgba(129,140,248,0.08)" },
  Training: { label: "TR", color: "#34d399", bg: "rgba(52,211,153,0.08)" },
  "Field setup": { label: "FS", color: "#f87171", bg: "rgba(248,113,113,0.08)" },
  Uniforms: { label: "UN", color: "#f472b6", bg: "rgba(244,114,182,0.08)" },
  Vehicle: { label: "VH", color: "#fb923c", bg: "rgba(251,146,60,0.08)" },
  Shadowing: { label: "SH", color: "#2dd4bf", bg: "rgba(45,212,191,0.08)" },
  "Final checks": { label: "FC", color: "#4ade80", bg: "rgba(74,222,128,0.08)" },
};
...
.sa-login-mascot{width:120px;height:120px;margin:0 auto 16px;object-fit:contain}
...
.sa-field{width:100%;padding:14px 16px;border:1px solid #1e2a3a;border-radius:10px;background:#0d1320;color:#e2e8f0;font-size:15px;font-family:inherit;outline:none;margin-bottom:12px;transition:border-color .2s;box-sizing:border-box}
...
.sa-lvl-rank{font-family:'Archivo Black',sans-serif;font-size:22px;line-height:1;color:#c8a84e}
.sa-lvl-inner small{font-size:8px;font-weight:700;color:#7a8599;letter-spacing:.5px;margin-top:2px;text-transform:uppercase}
...
.sa-badge{width:36px;height:36px;border-radius:8px;background:rgba(200,168,78,0.12);border:1px solid rgba(200,168,78,0.25);display:flex;align-items:center;justify-content:center}
.sa-badge img{width:24px;height:24px;object-fit:contain}
...
.sa-ci{width:36px;height:36px;border-radius:8px;display:flex;align-items:center;justify-content:center;flex-shrink:0;font-family:'Archivo Black',sans-serif;font-size:12px;letter-spacing:1px}
...
.sa-fab{position:fixed;bottom:24px;right:24px;width:56px;height:56px;border-radius:16px;background:linear-gradient(135deg,#c8a84e,#a08530);color:#0a0e17;border:none;cursor:pointer;box-shadow:0 4px 24px rgba(200,168,78,0.35);z-index:300;display:flex;align-items:center;justify-content:center;transition:transform .15s;overflow:hidden}
.sa-fab:hover{transform:scale(1.08)}
.sa-fab img{width:40px;height:40px;object-fit:contain}
...
.sa-chat-h img{width:28px;height:28px;object-fit:contain}
...
.sa-chat-i{flex:1;padding:10px 14px;border-radius:10px;border:1px solid #1a2540;background:#0a0e17;color:#e2e8f0;font-size:13px;font-family:inherit;outline:none;box-sizing:border-box}
...
.sa-out:hover{border-color:#b91c1c;color:#b91c1c}`;
...
export function OnboardingApp() {
  ...
  /* LOGIN */
  if (!loggedIn) return (<><style>{CSS}</style><div className="sa-login"><div className="sa-login-card">
    <img src={MASCOT} alt="Spartan" className="sa-login-mascot" />
  ...
  if (!employee || !stats) return <div style={{padding:60,textAlign:"center",color:"#4a5568"}}>Loading...</div>;
  ...
  return (<><style>{CSS}</style><div className="sa">
    ...
    <div className="sa-hero">
      ...
      <div className="sa-hero-right">
        <div className="sa-lvl" style={{background:`conic-gradient(${cur.color} ${xpPct*3.6}deg,#1a2540 0)`}}>
          <div className="sa-lvl-inner">
            <span className="sa-lvl-rank">{cur.rank}</span>
            <small>{cur.name}</small>
          </div>
        </div>
        <div className="sa-xp">
          <div className="sa-xp-num">{stats.earnedXp}</div>
          <div className="sa-xp-label">XP earned</div>
          <div className="sa-xp-bar"><div className="sa-xp-fill" style={{width:`${xpPct}%`}}/></div>
          {next&&<div className="sa-xp-next">{next.min-stats.earnedXp} to {next.name}</div>}
        </div>
    ...
    <div className="sa-overall">
      <div className="sa-ring" style={{width:56,height:56}}>
        <svg viewBox="0 0 56 56" width="56" height="56"><circle cx="28" cy="28" r="22" fill="none" stroke="#1a2540" strokeWidth="5"/><circle cx="28" cy="28" r="22" fill="none" stroke="#c8a84e" strokeWidth="5" strokeDasharray={`${2*Math.PI*22}`} strokeDashoffset={`${2*Math.PI*22*(1-pct/100)}`} strokeLinecap="round" transform="rotate(-90 28 28)" style={{transition:"stroke-dashoffset 1s ease"}}/></svg>
        <div className="sa-ring-pct">{pct}%</div>
      </div>
      <div style={{flex:1}}>
        <div className="sa-overall-title">{stats.completedItems} of {stats.totalItems} tasks complete</div>
        <div className="sa-overall-sub">{stats.totalXp-stats.earnedXp} XP left to collect</div>
      </div>
      {achievements.length>0&&<div className="sa-badges">{achievements.map(a=><div key={a.badge_code} className="sa-badge" title={a.badge_description}><img src={MASCOT} alt={a.badge_name}/></div>)}</div>}
    </div>

    {nextMissions.length>0&&<div className="sa-missions"><h3>NEXT UP</h3>{nextMissions.map(m=>{const mt=CAT_META[m.category]||{label:"--",color:"#c8a84e"};return(<div key={m.id} className="sa-mi" onClick={()=>setExpandedCat(m.category)}><div className="sa-mi-dot" style={{background:mt.color}}/><div className="sa-mi-text">{m.title}</div><div className="sa-mi-cat">{m.category}</div><div className="sa-mi-xp">+{m.xp_value}</div></div>);})}</div>}

    <div className="sa-label">YOUR MISSIONS</div>
    <div className="sa-grid">
      {Object.entries(categories).map(([cn,cat])=>{const o=expandedCat===cn;const mt=CAT_META[cn]||{label:"--",color:"#c8a84e",bg:"rgba(200,168,78,0.08)"};const cp=cat.total>0?Math.round(100*cat.done/cat.total):0;const r=14;const ci=2*Math.PI*r;return(
        <div key={cn} className={`sa-cat${o?" open":""}`} style={{borderColor:o?mt.color:undefined}}>
          <div className="sa-ch" onClick={()=>setExpandedCat(o?null:cn)}>
            <div className="sa-ci" style={{background:mt.bg,color:mt.color}}>{mt.label}</div>
    ...
    <button className="sa-fab" onClick={()=>{setShowChat(!showChat);if(!showChat&&!chatMessages.length)setChatMessages([{role:"assistant",content:`Hey ${employee.name.split(" ")[0]}! I'm your Spartan coach. What do you need help with?`}]);}}>
      {showChat?<span style={{fontSize:22,fontWeight:700}}>{"\u2715"}</span>:<img src={MASCOT_R} alt="Coach"/>}
    </button>

    {showChat&&<div className="sa-chat">
      <div className="sa-chat-h"><img src={LOGO_NAV} alt="Spartan"/><div><b>Spartan Coach</b><small>AI assistant</small></div></div>
