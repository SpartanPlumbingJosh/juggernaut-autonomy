...
async def track_experiment_revenue(
    execute_sql: Callable[[str], Dict[str, Any]],
    log_action: Callable[..., Any],
    experiment_id: str,
    amount: Decimal,
    ...
    """Track revenue generated by an experiment."""
    try:
        engine = RevenueEngine(execute_sql, log_action)
        ...
        # Update experiment's revenue tracking
        await execute_sql(
            f"""
            UPDATE experiments
            SET revenue_generated = COALESCE(revenue_generated, 0) + {int(amount * 100)},
                updated_at = NOW()
        ...
        await log_action(
            "experiment.revenue_tracking_failed",
            f"Failed to track revenue for experiment {experiment_id}: {str(e)}",
            level="error",
            error_data={
