...
class EndpointVerifier:
        ...
        if isinstance(endpoint_def, str):
            ...
            if not isinstance(endpoint_def, dict):
                return EndpointResult(
                    ...
                    error="Endpoint definition must be a JSON object",
                )
    ...
    def _verify_http(self, definition: Dict[str, Any]) -> EndpointResult:
        ...
        retry_count = max(1, int(definition.get("retry_count", 1)))
    ...
    def _verify_composite(self, definition: Dict[str, Any]) -> EndpointResult:
        ...
        for i, check_def in enumerate(checks):
            raw_type = check_def.get("type", "unknown")
            check_type = str(raw_type).lower()
